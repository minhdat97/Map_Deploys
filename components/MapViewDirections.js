...
class MapViewDirections extends Component {
  ...
  fetchRoute(
    ...
    if (typeof directionsServiceBaseUrl === "string") {
      url += `?origin=${origin}&waypoints=${waypoints}&destination=${destination}&key=${apikey}&mode=${mode.toLowerCase()}&language=${language}&units=metric&region=${region}&departure_time=now&alternatives=true`;
    }
    console.log("url", url);
    ...
    return fetch(url)
      ...
      .then(json => {
        ...
        if (json.routes.length) {
          ...
          const newCoordsArray = [...this.state.coordsArray, json.routes.legs];
  ...
  render() {
    ...
    console.log(this.state.coordsArray.length);
